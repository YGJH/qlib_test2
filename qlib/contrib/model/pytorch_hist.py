...
import torch
from torch.utils.data import TensorDataset, DataLoaderimport torch.nn as nn
...
class HIST(Model):
    ...
    def train_epoch(self, x_train, y_train, stock_index):
        ...
        total_loss = 0.0
        for batch_idx, (x_batch, y_batch, idx_batch) in enumerate(self.train_loader):
            # 1) 检查输入和标签有无 NaN
            if torch.isnan(x_batch).any() or torch.isnan(y_batch).any():
                print(
                    f"⚠️ Batch {batch_idx} contains NaN. x_batch.mean={x_batch.mean().item()}, y_batch.mean={y_batch.mean().item()}"
                )
                return
            # 2) 前向
            pred = self.HIST_model(x_batch.to(self.device), self.concept_matrix.to(self.device))
            loss = self.loss_fn(pred, y_batch.to(self.device))
            # 3) 检查 loss
            if torch.isnan(loss):
                print(f"⚠️ Loss is NaN at batch {batch_idx}. pred[:3]={pred[:3].detach().cpu()}, label[:3]={y_batch[:3]}")
                return
            # 4) 反向并梯度裁剪
            self.optimizer.zero_grad()
            with torch.autograd.detect_anomaly():
                loss.backward()
            torch.nn.utils.clip_grad_norm_(self.HIST_model.parameters(), max_norm=1.0)
            self.optimizer.step()
    ...
    def fit(
        ...
        # --- 新增：从 dataset 获取训练/验证 DataFrame 并定义 df_train, df_valid ---
        ...
        # --- 新增：加载并映射 stock_index ---
        stock_index_dict = np.load(self.stock_index, allow_pickle=True).item()
        df_train["stock_index"] = df_train.index.get_level_values("instrument").map(stock_index_dict)
        df_valid["stock_index"] = df_valid.index.get_level_values("instrument").map(stock_index_dict)
        ...
        # --- 新增：构建 PyTorch DataLoader ---
        # stack features: list of arrays [d_feat, T] -> np.ndarray [N, d_feat, T]
        feat_train = np.stack(x_train.values)
        # 将 label 转为 numpy 数组
        lab_train = y_train.values if isinstance(y_train, np.ndarray) else np.array(y_train.values)
        idx_train = np.array(stock_index_train.tolist())
        train_ds = TensorDataset(
            torch.tensor(feat_train, dtype=torch.float32),
            torch.tensor(lab_train, dtype=torch.float32).unsqueeze(-1),
            torch.tensor(idx_train, dtype=torch.long),
        )
        self.train_loader = DataLoader(train_ds, batch_size=self.kwargs.get("batch_size", 128), shuffle=True)

        feat_valid = np.stack(x_valid.values)
        lab_valid = y_valid.values if isinstance(y_valid, np.ndarray) else np.array(y_valid.values)
        idx_valid = np.array(stock_index_valid.tolist())
        valid_ds = TensorDataset(
            torch.tensor(feat_valid, dtype=torch.float32),
            torch.tensor(lab_valid, dtype=torch.float32).unsqueeze(-1),
            torch.tensor(idx_valid, dtype=torch.long),
        )
        self.valid_loader = DataLoader(valid_ds, batch_size=self.kwargs.get("batch_size", 128), shuffle=False)
        ...
        # 尝试加载已有模型，如果成功则直接返回；否则继续训练新模型
        if self.model_path and os.path.exists(self.model_path):
            try:
                self.logger.info(f"Loading pretrained model from {self.model_path} …")
                state_dict = torch.load(self.model_path, map_location=self.device)
                self.HIST_model.load_state_dict(state_dict)
                self.logger.info("✅ Pretrained model loaded successfully, skipping training.")
                self.fitted = True
                return
            except Exception as e:
                self.logger.warning(f"⚠️ Failed to load pretrained model: {e}\nProceeding to train a new model.")
